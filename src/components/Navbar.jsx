import React from "react";
import { Mail, Code } from "lucide-react";

// Inline SVG for Github to prevent Lucide version conflicts
const Github = ({ size = 24, className }) => (
  <svg
    viewBox="0 0 24 24"
    width={size}
    height={size}
    stroke="currentColor"
    strokeWidth="2"
    fill="none"
    strokeLinecap="round"
    strokeLinejoin="round"
    className={className}
  >
    <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" />
    <path d="M9 18c-4.51 2-5-2-7-2" />
  </svg>
);

// Inline SVG for Twitter to prevent Lucide version conflicts
const Twitter = ({ size = 24, className }) => (
  <svg
    viewBox="0 0 24 24"
    width={size}
    height={size}
    stroke="currentColor"
    strokeWidth="2"
    fill="none"
    strokeLinecap="round"
    strokeLinejoin="round"
    className={className}
  >
    <path d="M22 4s-.7 2.1-2 3.4c1.6 10-9.4 17.3-18 11.6 2.2.1 4.4-.6 6-2C3 15.5.5 9.6 3 5c2.2 2.6 5.6 4.1 9 4-.9-4.2 4-6.6 7-3.8 1.1 0 3-1.2 3-1.2z" />
  </svg>
);

export default function Navbar({ activeTab, setActiveTab }) {
  const tabs = [
    { id: "home", label: "首页" },
    { id: "blog", label: "博客" },
    { id: "photos", label: "相册日记" },
    { id: "projects", label: "开源项目" },
    { id: "agent", label: "AI 智能" },
    { id: "about", label: "关于" },
  ];

  return (
    <header className="navbar-container">
      <div className="navbar-glass">
        {/* Logo / Brand */}
        <div className="navbar-brand" onClick={() => setActiveTab("home")}>
          <Code className="brand-icon" />
          <span className="brand-text">
            Aohs<span className="accent-dot">.</span>
            <span className="brand-sub">DEV</span>
          </span>
        </div>

        {/* Navigation Tabs */}
        <nav className="navbar-tabs">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              className={`navbar-tab-btn ${activeTab === tab.id ? "active" : ""}`}
              onClick={() => setActiveTab(tab.id)}
            >
              {tab.label}
              {activeTab === tab.id && <span className="tab-indicator" />}
            </button>
          ))}
        </nav>

        {/* Social Quick Links */}
        <div className="navbar-socials">
          <a
            href="https://github.com"
            target="_blank"
            rel="noopener noreferrer"
            className="social-icon-btn"
            title="GitHub"
          >
            <Github size={18} />
          </a>
          <a
            href="https://twitter.com"
            target="_blank"
            rel="noopener noreferrer"
            className="social-icon-btn"
            title="Twitter"
          >
            <Twitter size={18} />
          </a>
          <a
            href="mailto:contact@aohs.dev"
            className="social-icon-btn"
            title="Email"
          >
            <Mail size={18} />
          </a>
        </div>
      </div>
    </header>
  );
}
